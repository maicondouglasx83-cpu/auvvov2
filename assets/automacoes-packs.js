...
(function () {
  ...
  async function fetchPacks() {
    ...
    const res = await fetch(API + '?action=crm_list_pack_templates');
  ...
  async function applyPack(pack) {
    ...
    try {
      ...
      for (const flowDef of flowDefs) {
        ...
        ffd.append('is_active', '0');
        if (pid) ffd.append('pipeline_id', String(pid));
        const fr = await fetch(API, { method: 'POST', body: ffd });
        const fdRes = await fr.json();
        if (fdRes.error) {
          console.warn('Fluxo não salvo:', flowDef.name, fdRes.message);
        }
      }
