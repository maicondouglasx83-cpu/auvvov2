...
(function () {
  ...
  window.FlowConfig = {
    TRIGGER_LABELS: {
      ...
      stage_enter: 'Lead entra no estagio',
      tag_added: 'Tag adicionada',
      contact_created: 'Lead criado',
      webhook_received: 'Webhook / integracao',
    ...
    TRIGGER_OPTIONS: [
      ...
      },
      {
        group: 'CRM & funil',
        items: [
          { value: 'stage_enter', icon: 'ph-columns', color: '#4338ca', label: 'Entrada em estagio', hint: 'Kanban / pipeline' },
      ...
      {
        group: 'Integracoes & LTV',
        items: [
          { value: 'webhook_received', icon: 'ph-plugs-connected', color: '#7c3aed', label: 'Webhook de entrada', hint: 'Hotmart, formulario, etc.' },
    ...
    ACTION_LABELS: {
      ...
      set_memory: 'Memoria IA',
      google_sheets_append: 'Google Sheets',
      http_preset: 'HTTP preset',
      brain_mission: 'Missao para o cerebro',
      clear_brain_mission: 'Limpar missao IA',
    },
    ...
    MESSAGE_VARS: [
      ...
      { key: 'estagio', label: 'Estagio' },
      { key: 'agente', label: 'Agente' },
      { key: 'mensagem', label: 'Msg gatilho' },
      { key: 'mensagens_hoje', label: 'Msgs hoje' },
      { key: 'sessao', label: 'Sessao (8)' },
      { key: 'ultima_sessao', label: 'Ultima sessao' },
      { key: 'tags', label: 'Tags' },
      { key: 'memoria.origem', label: 'Memoria' },
    ],
    ...
    get ACTION_OPTIONS() {
      return Object.keys(this.ACTION_LABELS).map((k) => {
        ...
        return { value: k, label: this.ACTION_LABELS[k] || k, icon: meta.icon, color: meta.color, hint: k === 'send_whatsapp' ? 'Texto + variaveis' : '' };
      });
    },
    ...
    get ACTION_OPTIONS_GROUPED() {
      ...
      return [
        ...
        { group: 'CRM & cerebro', items: opts.filter((o) => ['move_stage', 'add_tag', 'remove_tag', 'set_memory', 'brain_mission', 'clear_brain_mission'].includes(o.value)) },
        { group: 'Integracoes', items: opts.filter((o) => ['call_webhook', 'http_preset', 'google_sheets_append'].includes(o.value)) },
      ];
    },
  };
})();
