...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    // Verificar CSRF em todas as submissões POST
    csrf_verify();
    $action = $_POST['action'] ?? '';

    // ============================================================
    // BLUEPRINTS (modelos reutilizáveis)
    // ============================================================
    ...
    if ($audio_voice === '' && $prevAgent) {
        $audio_voice = (string)($prevAgent['audio_voice'] ?? '');
    }

    $handoff_rules = trim((string)($_POST['handoff_rules'] ?? ''));
    if ($handoff_rules === '' && $prevAgent) {
        $handoff_rules = (string)($prevAgent['handoff_rules'] ?? '');
    }
    if ($handoff_rules === '') {
        $handoff_rules = 'humano, atendente, suporte';
    }
    ...
    if ($action == 'create_agent') {
        if (!auvvo_agent_name_is_valid($name)) {
            header('Location: agentes?edit=new&error=name_required');
            exit;
        }
        try {
            ...
            header('Location: agentes?edit=' . $new_id . '&success=created');
            exit;
        } catch (PDOException $e) {
            error_log('[Auvvo] create_agent: ' . $e->getMessage());
            header('Location: agentes?error=save_failed&detail=' . urlencode($e->getMessage()));
            exit;
        }
    }
    if ($action == 'update_agent') {
        $agent_id = intval($_POST['agent_id'] ?? 0);
        if ($agent_id <= 0) {
            header('Location: agentes?error=save_failed');
            exit;
        }
        if (!$prevAgent) {
            header('Location: agentes?error=save_failed');
        ...
        if (!auvvo_agent_name_is_valid($name)) {
            header("Location: agentes?edit={$agent_id}&error=name_required");
            exit;
        }
        ...
        try {
            ...
            header("Location: agentes?edit={$agent_id}&error=save_failed&detail=" . urlencode($e->getMessage()));
            exit;
        }
        $redirect_tab = max(0, intval($_POST['redirect_tab'] ?? 0));
        if ($wasDraft) {
            header("Location: agentes?edit={$agent_id}&success=created&tab={$redirect_tab}");
            exit;
        }
        header("Location: agentes?edit={$agent_id}&tab={$redirect_tab}&success=updated");
        exit;
    }
    if ($action == 'delete_agent') {
        $agent_id = intval($_POST['agent_id'] ?? 0);

        // Buscar e deletar arquivos físicos de conhecimento antes de remover do banco
        $stmt_kb = $pdo->prepare(
            "SELECT file_name, file_type FROM knowledge_base WHERE agent_id = ?"
        );
        ...
        // Deleta agente (cascade remove knowledge_base via FK)
...
if (isset($_GET['edit'])) {
    if ($_GET['edit'] === 'new') {
        $stmt = $pdo->prepare("INSERT INTO agents (user_id, name, agent_type, role, model, temperature, max_tokens, response_delay, status) VALUES (?, 'Rascunho Novo Agente', 'vendedor', 'Atendente', 'openrouter/owl-alpha', 0.7, 1000, 2, 'draft')");
...
$stmt = $pdo->prepare(
    "SELECT id, name, agent_type, status, model, created_at
...
function agent_model_display_label(string $model): string {
    return $model === 'auvvo-ai' ? 'Auvvo AI' : $model;
}
?>
<!DOCTYPE html>
<html lang="<?= lang_html() ?>">
...
.brain-cap-item.off .brain-cap-badge{background:#F3F4F6;color:#6B7280}
...
<div class="brain-caps-panel">
  ...
  <div class="brain-caps-grid">
    ...
    <div class="brain-cap-item <?= $cap['connected'] ? 'on' : 'off' ?>">
      <div class="brain-cap-head">
        ...
        <span class="brain-cap-badge"><?= $cap['connected'] ? 'Ativo' : 'Pendente' ?></span>
      </div>
  ...
  <div class="brain-caps-footer">Conectar integrações: <a href="configuracoes">Configurações</a> · <a href="integracoes">Integrações</a> · WhatsApp: <a href="automacoes">Automações</a></div>
</div>
...
<span style="font-size:.875rem">Próximo passo: conecte o WhatsApp em <a href="automacoes">Automações</a> e crie uma regra de gatilho.</span>
<?php endif; ?>
</div>
<?php endif; ?>
                    ...
                    <div class="form-group">
                      ...
                      <div class="form-select-wrapper">
                          <select name="agent_type" class="form-control">
                            ...
                            <option value="suporte" <?=(($a['agent_type']??'')==='suporte')?'selected':''?>>Suporte Técnico</option>
                    ...
                    <div class="form-group" style="margin:0">
                      ...
                      <div class="form-select-wrapper">
                        <select name="model" class="form-control">
                          <?php
                          $known_models = ['gpt-4o','gpt-4o-mini','gpt-4-turbo','gpt-3.5-turbo','gemini-flash-latest','openrouter/owl-alpha'];
          ...
          <div style="display:flex;align-items:center;gap:16px">
            ...
            <div>
              <div style="font-weight:600;font-size:.9375rem;color:var(--text-primary);margin-bottom:2px;display:flex;align-items:center;gap:8px">
                ${tag ? `<span class="badge badge-gray" style="font-size:.65rem">${tag}</span>` : ''}
                <span>${name}</span>
              </div>
...
async function generateWithGemini() {
  ...
  // Campos estruturados do wizard (prompts)
  ...
  // UI: loading state
  btn.disabled = true;
  btn.innerHTML = '<span style="font-size:1.125rem">⏳</span> Gerando...';
  ...
  try {
    ...
    if (data.error) {
      alert('Erro do Gemini: ' + data.message);
    } else {
      if (ta) {
        ta.value = data.prompt;
        // Feedback visual no textarea
        ta.style.borderColor = '#4285F4';
        ta.style.boxShadow = '0 0 0 3px rgba(66,133,244,.2)';
        setTimeout(() => { ta.style.borderColor = ''; ta.style.boxShadow = ''; }, 2500);
        ta.scrollIntoView({ behavior: 'smooth', block: 'center' });
        // Banner de sucesso
        let banner = document.getElementById('gemini-success-banner');
        if (!banner) {
          banner = document.createElement('div');
          banner.id = 'gemini-success-banner';
        ...
        banner.innerHTML = '<i class="ph-bold ph-check-circle" style="font-size:1.25rem"></i> Prompt gerado com sucesso! Revise o texto acima e clique em <strong style="margin-left:4px">Salvar Agente</strong>.';
        banner.style.display = 'flex';
        setTimeout(() => { if(banner) banner.style.display = 'none'; }, 8000);
      }
    }
  } catch (e) {
    alert('Erro de conexão ao chamar Gemini.');
  } finally {
    btn.disabled = false;
    btn.innerHTML = '<span style="font-size:1.125rem">✨</span> Gerar com Gemini';
