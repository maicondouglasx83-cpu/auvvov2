...
export async function verifyInternalHmac() {
  ...
  // Transient errors (502, 503, 504, timeout, network) — retry, don't kill worker
  console.error('[auvvo-worker] HMAC verify transient:', status, data.error || 'unknown');
  if (status === 504 || status === 503) {
    console.error('[auvvo-worker] Retrying HMAC verify in 5s...');
    await new Promise(r => setTimeout(r, 5000));
    return verifyInternalHmac();
  }
  return true; // assume ok for other unexpected responses
}
