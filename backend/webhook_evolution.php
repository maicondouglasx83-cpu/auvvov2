...
// ============================================================
if ($event_type === 'MESSAGE' && !($data['Info']['IsFromMe'] ?? true)) {
    ...
    $lock_peer = $peer_digits !== '' ? $peer_digits : ('h' . substr(md5($remote_jid), 0, 12));
    ...
    // ============================================================
    // IA: inline (padrão) ou fila (WEBHOOK_AI_MODE=queue + worker)
    // ============================================================
    if (auvvo_webhook_ai_use_queue()) {
        require_once __DIR__ . '/conversation_events.inc.php';

        $dedupeKey = evolutionInboundDedupeKey((int) $agent['id'], $data, $remote_jid, $body, $lock_peer);

        if (rand(1, 200) === 1) {
            try {
                $pdo->prepare("DELETE FROM webhook_message_dedup WHERE created_at < DATE_SUB(NOW(), INTERVAL 7 DAY)")->execute();
            } catch (PDOException $e) {
            }
        }
    ...
    $lkStmt = $pdo->prepare('SELECT GET_LOCK(?, 180)');
    $lkStmt->execute([$evoLock]);
    $lockGot = $lkStmt->fetchColumn();
    if ($lockGot === false || (int) $lockGot !== 1) {
        error_log('[Evolution] GET_LOCK timeout (inline): ' . $evoLock);
        auvvo_webhook_tracelog('exit', ['reason' => 'lock_busy_inline', 'lock' => $evoLock]);
        http_response_code(200);
        echo json_encode(['ok' => true, 'info' => 'lock busy']);
        exit;
    }
    ...
    try {
        ...
        try {
            ...
            )->execute([$agent['id'], $dedupeKey]);
        } catch (PDOException $e) {
            auvvo_evolution_release_lock($pdo, $evoLock);
            auvvo_webhook_tracelog('exit', ['reason' => 'dedup_duplicate', 'key_preview' => mb_substr($dedupeKey, 0, 40)]);
            http_response_code(200);
            echo json_encode(['ok' => true, 'info' => 'duplicate webhook (dedup key)']);
            exit;
        }
        if (rand(1, 100) === 1) {
            try {
...
if (in_array($event_type, ['CONNECTED', 'PAIRSUCCESS', 'LOGGEDOUT', 'OFFLINESYNCCOMPLETED', 'CONNECTION'], true)) {
    if ($event_type === 'OFFLINESYNCCOMPLETED') {
        ...
        require_once __DIR__ . '/whatsapp_connections.inc.php';
        auvvo_whatsapp_update_connection_status($pdo, $webhook_instance_token, $instance_slug, $db_status);
    }
}
...
/**
 * Busca o histórico de mensagens de um contato para o contexto do modelo (Chat ou Gemini).
 * Retorna array de ['role' => 'user'|'assistant', 'content' => '...']
 * ordenado do mais antigo para o mais recente.
 * Só inclui turnos de IA já finalizados (com resposta gravada); pendings não entram — o texto
 * da mensagem atual é passado separadamente a callOpenAI, evitando duplicar o user e “perguntas sem resposta”.
 *
 * @param PDO    $pdo          Conexão PDO
 * @param int    $agent_id     ID do agente
 * @param string $primary_jid   JID canônico (ex.: dígitos@s.whatsapp.net)
 * @param int    $maxMessages   Máximo de linhas de log a considerar
 * @param string $alt_jid       Outro JID visto no webhook (ex.: @lid) para unificar histórico
 * @param string $peer_digits   Dígitos do telefone, se extraíveis
 */
...
function getConversationHistoryRaw(PDO $pdo, int $agent_id, string $primary_jid, int $maxMessages = 10, string $alt_jid = '', string $peer_digits = ''): array {
    try {
        // Histórico em ordem cronológica user/assistant — compatível com Chat Completions e com
        // Gemini generateContent (papéis user/model em alternância). Ver https://ai.google.dev/api
        $conds = ['contact_jid = ?'];
        ...
        // Os resultados estão do mais recente para o mais antigo — inverter para ordem cronológica
        $rows = array_reverse($rows);

        // Montar array de mensagens no formato OpenAI
        $messages = [];
        foreach ($rows as $row) {
            // Mensagem do usuário
            ...
            if (!empty($row['response_msg'])) {
                $messages[] = ['role' => 'assistant', 'content' => $row['response_msg']];
            }
        }
        ...
        // Silencia se a tabela não existir ou der erro — melhor responder sem histórico do que falhar
        return [];
    }
}
...
function extractGcalDirective(string $aiText): ?array {
    $pattern = '/\\[\\[GCAL_EVENT\\]\\]\\s*(\\{[\\s\\S]*\\})\\s*$/u';
    if (!preg_match($pattern, $aiText, $m)) return null;

    $json = trim($m[1] ?? '');
    $payload = json_decode($json, true);
    if (!is_array($payload)) return null;

    $clean = trim(preg_replace($pattern, '', $aiText));
