...
class PaymentGateway {
    ...
    // ── Gateways legados (placeholders) ─────────────────────────────────────
    private function mercadoPago($u, $p, $method) {
        return [
            'status'         => 'success',
            'gateway'        => 'mercadopago',
            'transaction_id' => 'MP_' . time(),
            'message'        => "Pagamento Mercado Pago ({$method}) gerado.",
        ];
    }
    ...
    private function pagSeguro($u, $p, $method) {
        return [
            'status'         => 'success',
            'gateway'        => 'pagseguro',
            'transaction_id' => 'PS_' . time(),
            'message'        => "Pagamento PagSeguro ({$method}) gerado.",
        ];
    }
    ...
    private function cielo($u, $p, $method) {
        return [
            'status'         => 'success',
            'gateway'        => 'cielo',
            'transaction_id' => 'CIELO_' . time(),
            'message'        => "Transação Cielo ({$method}) aprovada.",
        ];
    }
    ...
    private function efi($u, $p, $method) {
        return [
            'status'         => 'success',
            'gateway'        => 'efi',
            'transaction_id' => 'EFI_' . time(),
            'message'        => "Cobrança Efí ({$method}) gerada.",
        ];
    }
}
?>
