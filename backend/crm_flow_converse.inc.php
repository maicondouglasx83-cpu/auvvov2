...
function auvvo_flow_run_converse_node(
    ...
    if ($simulate) {
        ...
        auvvo_flow_converse_save($pdo, $userId, (string) $contact['jid'], $session);
        auvvo_contact_memory_merge($pdo, $userId, (string) $contact['jid'], ['_brain_mission' => $instructionsRendered]);
        $contact = auvvo_crm_hydrate_contact($pdo, $userId, $contact);
    }
...
function auvvo_flow_converse_inbound(
    ...
    if (!$reply['ok']) {
        return ['handled' => true, 'ai_handled' => false, 'ended' => false];
    }

    $sess['turns'] = (int) ($sess['turns'] ?? 0) + 1;
    auvvo_flow_converse_save($pdo, $userId, (string) ($contact['jid'] ?? ''), $sess);
    auvvo_automation_mark_ai_handled();
...
function auvvo_flow_converse_reply(
    ...
    $peerDigits = auvvo_whatsapp_peer_digits($jid);
