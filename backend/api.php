...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    $token_ok = !empty($_SESSION['csrf_token']) && hash_equals($_SESSION['csrf_token'], $_POST['csrf_token'] ?? '');
    if (!$token_ok) {
...
switch ($action) {
    ...
    case 'create_whatsapp_connection': {
        ...
        $pdo->prepare(
            'INSERT INTO whatsapp_connections (user_id, name, status, default_agent_id) VALUES (?, ?, ?, ?)'
        )->execute([$user_id, $name, 'offline', $defaultAgent > 0 ? $defaultAgent : null]);
    ...
    case 'update_whatsapp_connection': {
        ...
        $pdo->prepare(
            'UPDATE whatsapp_connections SET name = ?, default_agent_id = ? WHERE id = ? AND user_id = ?'
        )->execute([$name, $defaultAgent > 0 ? $defaultAgent : null, $cid, $user_id]);
    ...
    case 'evolution_status': {
        ...
        json_out(['state' => $state, 'label' => $labels[$state] ?? 'Verificando...', 'raw' => $status, 'connection_id' => (int) $conn['id']]);
    }
    ...
    case 'gemini_generate_prompt': {
        ...
        $type_labels = [
            'vendedor'  => 'Vendedor / Fechador de Negócios',
        ...
        $prompt = "Você é um especialista em atendimento e atua como {$type_label}.\n\n";
        ...
        if ($organizacao)   $prompt .= "- Tipo de Organização: {$organizacao}\n";
        if ($publico_alvo)  $prompt .= "- Seu Público-Alvo: {$publico_alvo}\n";
        if ($objetivo_agente) $prompt .= "- Seu Objetivo Principal: {$objetivo_agente}\n\n";

        $prompt .= "### DIRETRIZES DE COMPORTAMENTO\n";
        $prompt .= "- Tom de Voz: Você deve se comunicar de forma {$tone}.\n";
        if ($abertura)      $prompt .= "- Mensagem de Abertura sugerida (use como base): \"{$abertura}\"\n";
        $prompt .= "- Responda de forma clara, objetiva e humana. Seja empático e prestativo.\n\n";

        $prompt .= "### INFORMAÇÕES ESPECÍFICAS DA FUNÇÃO\n";
        if ($agent_type === 'vendedor') {
            if ($products)     $prompt .= "- Produtos/Serviços e Preços:\n  {$products}\n";
            if ($payments)     $prompt .= "- Condições de Pagamento:\n  {$payments}\n";
            if ($diferenciais) $prompt .= "- Diferenciais (Use para quebrar objeções):\n  {$diferenciais}\n";
        } elseif ($agent_type === 'atendente') {
            if ($horario)  $prompt .= "- Horário de Atendimento:\n  {$horario}\n";
            if ($politica) $prompt .= "- Política de Trocas/Prazos/Termos:\n  {$politica}\n";
            ...
            if ($setores) $prompt .= "- Setores para Transferência:\n  {$setores}\n";
        }

        $prompt .= "\n### REGRAS E RESTRIÇÕES\n";
        if ($restricoes) $prompt .= "- {$restricoes}\n";
        if ($extra_rules) $prompt .= "- {$extra_rules}\n";
        $prompt .= "- Nunca invente informações que não foram fornecidas. Se não souber responder, seja honesto e direcione o usuário para um humano se possível.\n";

        json_out(['error' => false, 'prompt' => trim($prompt)]);
    }
    ...
    case 'inbound_webhook_delete': {
        require_once __DIR__ . '/migrations.php';
        $id = (int) ($_POST['id'] ?? 0);
        $pdo->prepare('DELETE FROM inbound_webhook_field_maps WHERE webhook_id = ?')->execute([$id]);
