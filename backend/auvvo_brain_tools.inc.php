...
function auvvo_brain_parse_response(string $aiText): array
{
    ...
    $jsonBlob = null;
    if (preg_match_all('/\[\[AUVO_ACTIONS\]\]\s*(\[[\s\S]*?\])\s*/u', $text, $allMatches, PREG_SET_ORDER)) {
        $last = $allMatches[count($allMatches) - 1];
        $jsonBlob = trim($last[1] ?? '');
    } elseif (preg_match('/\[\[AUVO_ACTIONS\]\]\s*(\[[\s\S]*\])\s*$/u', $text, $m)) {
        $jsonBlob = trim($m[1] ?? '');
    }
    if ($jsonBlob !== null && $jsonBlob !== '') {
        ...
        $text = trim(preg_replace('/\[\[AUVO_ACTIONS\]\]\s*\[[\s\S]*?\]\s*/u', '', $text));
        $text = trim(preg_replace('/\[\[AUVO_ACTIONS\]\]\s*$/u', '', $text));
    }
...
function auvvo_brain_build_prompt_section(PDO $pdo, array $agent, array $company): string
{
    ...
    $lines[] = 'Quando uma ação for necessária e você tiver dados confirmados, inclua NA ÚLTIMA LINHA da resposta:';
    $lines[] = '[[AUVO_ACTIONS]]';
    $lines[] = '[ {"tool":"nome.da.ferramenta","payload":{...}}, ... ]';
    $lines[] = 'Legado ainda aceito para agenda: [[GCAL_EVENT]]{...json...} na última linha.';
    $lines[] = '';
